...
TEST_F(KMAlgoTest, normal_function_test)
{
    ...
    std::cout << "process use:" << std::chrono::duration_cast<std::chrono::microseconds>(time_after - time_before).count() << " us" << std::endl;

    cout << "total cost: " << cost << " match pairs: 1-" << assignment[0] << " 2-" << assignment[1] << " 3-"<< assignment[2] << endl;
