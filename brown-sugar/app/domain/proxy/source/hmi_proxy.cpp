...
{
	LOG(INFO) << "server receive msg from IPC: " << static_cast<int>(msgType);
	...
	case MsgType::IPC_GNSS_DATA:
	{
		...
		m_HostVehCIPVInfo.data.heading = round(gnssData->headingAngle * 100) / 100;
		LOG(INFO) << " m_HostVehCIPVInfo.data.heading " << m_HostVehCIPVInfo.data.heading;

		m_HostVehCIPVInfo.data.latitude = gnssData->latitude;
		m_HostVehCIPVInfo.data.longitude = gnssData->longitude;
	};
		break;
	...
	case MsgType::IPC_EVH:
	{
		...
		LOG(INFO) << "IPC_EVH from IPC: " 
			...
			<< "\n\tDe_ego_axMs2_f32: " << hostInfo->De_ego_axMs2_f32;
		m_HostVehCIPVInfo.data.speed = round(ADASMath::convertSpeedMs2Kmh(round(hostInfo->De_ego_vxMs_f32)));
		m_HostVehCIPVInfo.data.acceleration = round(sqrtf(powf(hostInfo->De_ego_axMs2_f32, 2)
...
void HmiProxy::DoServerWrite(T& msg)
{
	json j;
    to_json(j, msg);
	LOG(INFO) << "server send to PAD Body(T): " << j.dump() << " BodyLength: " << j.dump().size();
    m_Server.write(HmiTcpMessage(j.dump()));
}

void HmiProxy::HandlegSentryMessage(uint8_t* data, uint16_t len)
{
	std::string msg((char*)data);
	if (msg.empty() || msg.size() != len)
		return;
	//将{"data":null,"tag":1003}E变成格式正确的json串
	if (msg[len] != '\0')
		msg[len] = '\0';
	...
	LOG(INFO) << "client receive from gSentry(HMI) data: " << msg << " len: " << msg.size();
...
void HmiProxy::HandlePadMessage(uint8_t* data, uint16_t len)
{
	...
	HMI_PAD_TAG_DEF tag = HMI_PAD_TAG_DEF(j["tag"]);
	LOG(INFO) << " server receive data from pad: " << msg <<"  len: " << msg.size();
	// PAD连接请求响应(PAD给hiproxy发起连接请求，hmiproxy直接回复)
	if (HMI_PAD_TAG_DEF::HMI_TAG_PAD_CON_REQ == tag)
	{
		...
		// ADAS系统开关状态上报，系统每次开机需主动上报 目前默认值
	...
	case HMI_PAD_TAG_DEF::HMI_TAG_PAD_CACC_CC_REQ: {
		m_ToMcoreInfo.De_RequestCounter_u32 = m_Counter;
		m_ToMcoreInfo.De_CACC_Resume_u8 = true;
		m_ToMcoreInfo.De_Vset_f32 = j["setSpeed"];
		CDD_FUSION_EVENT_QUEUE.push({MsgType::IPC_HMI_CTRL, (const char *)&m_ToMcoreInfo , sizeof(SignalInput_HMI_BUS)});
		LOG(INFO) << "m_ToMcoreInfo.De_RequestCounter_u32: " << m_ToMcoreInfo.De_RequestCounter_u32
			<< "  m_ToMcoreInfo.De_CACC_Resume_u8" << (int)m_ToMcoreInfo.De_CACC_Resume_u8 
	...
	case HMI_PAD_TAG_DEF::HMI_TAG_PAD_CACC_TIME_REQ: {
		m_ToMcoreInfo.De_RequestCounter_u32 = m_Counter;
		m_ToMcoreInfo.De_TimeGapSet_f32 = j["timeHeadway"];
		CDD_FUSION_EVENT_QUEUE.push({MsgType::IPC_HMI_CTRL, (const char *)&m_ToMcoreInfo , sizeof(SignalInput_HMI_BUS)});
		LOG(INFO) << "m_ToMcoreInfo.De_RequestCounter_u32: " << m_ToMcoreInfo.De_RequestCounter_u32
	...
	case HMI_PAD_TAG_DEF::HMI_TAG_PAD_CAEB_COLLISION_SENS_REQ: {
		m_ToMcoreInfo.De_RequestCounter_u32 = m_Counter;
		m_ToMcoreInfo.De_FCW_SnvtySet_u8 = j["collisionSensitivity"];
		CDD_FUSION_EVENT_QUEUE.push({MsgType::IPC_HMI_CTRL, (const char *)&m_ToMcoreInfo , sizeof(SignalInput_HMI_BUS)});
		LOG(INFO) << "m_ToMcoreInfo.De_RequestCounter_u32: " << m_ToMcoreInfo.De_RequestCounter_u32
...
/*   数据精度未调整!!!!!!!!!!!!!!!!!!!
	...
	*/
	...
	{
		//目标车车速（绝对速度）
		m_HostVehCIPVInfo.data.objSpeed = round(ADASMath::convertSpeedMs2Kmh(
			ADASMath::convertSpeedI2F(v2xObj.speed)));       // =>目标车车速 单位km/h
		//目标距离（相对距离）
		PositionModel host_p;
		host_p.setLatitude(m_HostVehCIPVInfo.data.latitude);
		host_p.setLongitude(m_HostVehCIPVInfo.data.longitude);
		PositionModel obj_p;
		...
		//时距  // =>目标时距 单位s
		if(0 != (abs(ADASMath::convertSpeedKmh2Ms(m_HostVehCIPVInfo.data.speed) - ADASMath::convertSpeedI2F(
			v2xObj.speed))))
		{
			m_HostVehCIPVInfo.data.objDistance = round(m_HostVehCIPVInfo.data.followDistance / abs(
				(ADASMath::convertSpeedKmh2Ms(m_HostVehCIPVInfo.data.speed) 
		...
		m_HostVehCIPVInfo.data.relativeVeloc = round(abs(m_HostVehCIPVInfo.data.speed - ADASMath::convertSpeedMs2Kmh(
			ADASMath::convertSpeedI2F(v2xObj.speed))));
		// =>相对速度 单位km/h
...
void HmiProxy::SendV2XObjVehInfo()
{
	...
	for(uint32_t i = 0; i < ADAS_GSENTRY_OBJ_VEHI_NUM; i++)
	{
		...
		if(v2xVel.localId)
		{
			...
			tmpVel.speed = round(ADASMath::convertSpeedMs2Kmh(ADASMath::convertSpeedI2F(v2xVel.speed)));
