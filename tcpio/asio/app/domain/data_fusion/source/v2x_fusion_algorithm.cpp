...
namespace
{
    ...
    const double V2X_VRU_SIZE_DIAMETER = 0.01;
...
void V2xFusionAlgo::ProcessObjVehiInfo(uint8_t* buf, uint32_t len)
{
    ...
    CDDFusion::CddFusionRepo& fusion = DataRepo::GetInstance().GetCddFusionData();
    memset(&fusion.cddObjects[0], 0, sizeof(CDD_Fusion_ObjInfo_BUS) * ADAS_OBJ_VEH_INFO_NUM);
...
void V2xFusionAlgo::ProcessRoadPartInfo(uint8_t* buf, uint32_t len)
{
    ...
    CAN::HostVehiclePos& host = DataRepo::GetInstance().GetHostVehicle();
    CDDFusion::CddFusionRepo& fusion = DataRepo::GetInstance().GetCddFusionData();
    memset(&fusion.cddObjects[ADAS_OBJ_VEH_INFO_NUM], 0, sizeof(CDD_Fusion_ObjInfo_BUS) * MAX_PARTCIPANT_NUM);
    ...
    uint16_t i = 0;
    v2xData.objVehiNum = 0;
    while(v2xData.roadPartInfo.participants[i].ptcId != 0 && i < ADAS_GSENTRY_VRU_VEHI_NUM)
    {
        TransV2xVru2CddObj(v2xData.roadPartInfo.participants[i], host,
                             fusion.cddObjects[ADAS_GSENTRY_OBJ_VEHI_NUM + i]);
        // printf("recieve gSentry ObjVehiInfo msg!\n");
        v2xData.objVehiNum++;
        i++;
    }
...
void V2xFusionAlgo::TransV2xVru2CddObj(const V2X::Participant& raw, const CAN::HostVehiclePos& host, CDD_Fusion_ObjInfo_BUS& dest)
{
    /* 计算过程中，朝向角以camera为准，即车辆行驶方向与正北方向的夹角，逆时针为正 */
    dest.De_ID_u8 = raw.ptcId;
    dest.De_Timestamp_u32 = raw.timestamp_ms;
    dest.De_measurement_status_u8 = raw.source;

    
    dest.De_length_f32 = raw.size.objectLength * V2X_ACCELERATION_DIAMETER;
    dest.De_width_f32 = raw.size.objectWidth * V2X_ACCELERATION_DIAMETER;
    dest.De_height_f32 = raw.size.objectHight * V2X_ACCELERATION_DIAMETER;
    dest.De_Yaw_f32 = -raw.heading * 0.0125 * PI / PI_DEGREE;

    vector<float>&& displacement = TransWgs84ToVcsCoordinate(host, raw.position);
    dest.De_dx_f32 = displacement[0];
    dest.De_dy_f32 = displacement[1];

    const double hostHeadRad  = -host.objectHeadingAngle * PI / PI_DEGREE;
    vector<float>&& velocity = TransObjVcsToHostVcs(raw.speed_ms * V2X_VELOCITY_DIAMETER, 0,
                                                    hostHeadRad, dest.De_Yaw_f32);
    dest.De_vx_f32 = velocity[0] - host.speed;
    dest.De_vy_f32 = velocity[1] + 0.001;

    vector<float>&& acceleration = TransObjVcsToHostVcs(raw.accelSet.longi * V2X_ACCELERATION_DIAMETER, 
                                    raw.accelSet.lat * V2X_ACCELERATION_DIAMETER, hostHeadRad, dest.De_Yaw_f32);
    dest.De_ax_f32 = acceleration[0] - host.acc_x;
    dest.De_ay_f32 = acceleration[1] - host.acc_y;

    // 和原有source定义有冲突
    dest.De_source_u8 = raw.source;
    dest.De_life_time_u32 = 0xFFFF;
    dest.De_ObjectType_u8 = raw.ptcType;

    // Moving到Moving_Slowly：7.2km/h  ~ 2m/s
    float obj_velocity = raw.speed_ms * V2X_VELOCITY_DIAMETER;
    if (obj_velocity >= 2.0)
    {
        // moving
        dest.De_ObjectMovingStatus_u8 = 2;
    ...
    dest.De_conf_f32 = 1.0;
    dest.De_dxVariance_f32 = raw.posConfidence.pos;
    dest.De_dyVariance_f32 = raw.posConfidence.pos;
    dest.De_vxVariance_f32 = 0;
    dest.De_vyVariance_f32 = 0;
    dest.De_axVariance_f32 = 0;
    dest.De_ayVariance_f32 = 0;
