...
void ParseLineV2Msg(std::shared_ptr<google::protobuf::Message> message,
  ...
  lifetime.clear();
  LOG(INFO) << "Send lanes";
  gohigh::IpcM::getInstance().send(IPCM_FRAME_J3VISION_LANE, (void *)&out, sizeof(out));
}
...
static void ParseObstacles(const ObstacleProto::Obstacles *obstacles, gohigh::Obstacles * const result)
{
  ...
  result->timestamp = obstacles->header().time_stamp();
...
void ParsePedResultMsg(std::shared_ptr<google::protobuf::Message> message,
  ...
  LOG(INFO) << "Send Pedstrains";
  gohigh::IpcM::getInstance().send(IPCM_FRAME_J3VISION_PED, &result, sizeof(result));
}
...
void ParseVehicleresultMsg(std::shared_ptr<google::protobuf::Message> message,
  ...
  LOG(INFO) << "Send Vehilces";
  gohigh::IpcM::getInstance().send(IPCM_FRAME_J3VISION_VEHICLE, &result, sizeof(result));
}
